...
{
    public class ZuiderzeelandCatchImportTask : MuraCatchImportTask
    {
        public ZuiderzeelandCatchImportTask(
            ILogger<ZuiderzeelandCatchImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Vangst, Catch>(
                ...
                where organisatie.id = {MuraOrganizationIds.Zuiderzeeland}",
                cancellationToken);
        }

        protected override string GetPrefixedId(string id)
        {
            return id.AsOrganizationPrefixed(OrganizationNames.Zuiderzeeland);
        }

        protected override bool DatePassesOrganizationConstrain(DateTimeOffset date)
        {
            return date.PassesOrganizationDateConstrain(OrganizationNames.Zuiderzeeland);
        }
    }
}
