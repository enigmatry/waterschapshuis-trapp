...
{
    public class NonlCatchImportTask : MuraCatchImportTask
    {
        public NonlCatchImportTask(
            ILogger<NonlCatchImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Vangst, Catch>(
                ...
                where organisatie.id = {MuraOrganizationIds.Noordoostnederland}",
                cancellationToken);
        }

        protected override string GetPrefixedId(string id)
        {
            return id.AsOrganizationPrefixed(OrganizationNames.Noordoostnederland);
        }
