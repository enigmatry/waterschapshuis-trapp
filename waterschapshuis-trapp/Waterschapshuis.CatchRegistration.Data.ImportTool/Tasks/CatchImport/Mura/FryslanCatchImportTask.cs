...
{
    public class FryslanCatchImportTask : MuraCatchImportTask
    {
        public FryslanCatchImportTask(
            ILogger<FryslanCatchImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Vangst, Catch>(
                ...
                where organisatie.id = {MuraOrganizationIds.Fryslan}",
                cancellationToken);
        }

        protected override string GetPrefixedId(string id)
        {
            return id.AsOrganizationPrefixed(OrganizationNames.Fryslan);
        }
