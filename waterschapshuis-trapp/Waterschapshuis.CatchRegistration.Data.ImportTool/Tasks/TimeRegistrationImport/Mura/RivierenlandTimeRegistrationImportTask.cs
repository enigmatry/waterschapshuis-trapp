...
{
    public class RivierenlandTimeRegistrationImportTask : MuraTimeRegistrationImportTask
    {
        public RivierenlandTimeRegistrationImportTask(
            ILogger<RivierenlandTimeRegistrationImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Urenregistratie, TimeRegistration>(
                ...
                where organisatie.id = {MuraOrganizationIds.Rivierenland} order by u.datum",
                cancellationToken);
        }
