...
{
    public class BrabantTimeRegistrationImportTask : MuraTimeRegistrationImportTask
    {
        public BrabantTimeRegistrationImportTask(
            ILogger<BrabantTimeRegistrationImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Urenregistratie, TimeRegistration>(
                ...
                where organisatie.id = {MuraOrganizationIds.Brabant} order by u.datum",
                cancellationToken);
        }
