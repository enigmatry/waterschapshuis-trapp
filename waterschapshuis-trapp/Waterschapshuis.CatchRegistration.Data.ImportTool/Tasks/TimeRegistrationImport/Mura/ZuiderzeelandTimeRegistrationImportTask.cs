...
{
    public class ZuiderzeelandTimeRegistrationImportTask : MuraTimeRegistrationImportTask
    {
        public ZuiderzeelandTimeRegistrationImportTask(
            ILogger<ZuiderzeelandTimeRegistrationImportTask> logger,
        ...
        protected override async Task ExecutePgImportsAsync(CancellationToken cancellationToken)
        {
            await ExecutePgImportAsync<Urenregistratie, TimeRegistration>(
                ...
                where organisatie.id = {MuraOrganizationIds.Zuiderzeeland} order by u.datum",
