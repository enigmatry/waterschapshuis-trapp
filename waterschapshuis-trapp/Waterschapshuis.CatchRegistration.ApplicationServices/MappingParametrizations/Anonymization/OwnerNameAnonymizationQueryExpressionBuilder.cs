using System;
using System.Linq.Expressions;
using Waterschapshuis.CatchRegistration.DomainModel.Identity;
using Waterschapshuis.CatchRegistration.DomainModel.ReportData;

namespace Waterschapshuis.CatchRegistration.ApplicationServices.MappingParametrizations.Anonymization
{
    // we need to build EF core SQL compatible expression here
    public class OwnerNameAnonymizationQueryExpressionBuilder<TSource> where TSource : OwnReportData
    {
        // do not refactor parameter into constructor (as with standard builders),
        // this build is used by AutoMapper's projectTo and needs
        // to be in this exact format, also name "parameters" needs to be exactly the same as in the MappingProfiles
        public Expression<Func<TSource, string>> Build(MappingParameters parameters) =>
            src =>
                src.CreatedOn.HasValue &&
                src.OwnerId.HasValue &&
                src.CreatedOn.Value >= parameters.AnonymizationParameter.AnonymizeBefore &&
                (
                    parameters.AnonymizationParameter.SkipAnonymizationIfDateConditionIsMeet ||
                    parameters.AnonymizationParameter.CurrentUserId == src.OwnerId.Value ||

                        parameters.AnonymizationParameter.IsSeniorUser &&
                        parameters.AnonymizationParameter.CurrentUserOrganizationId == src.OrganizationId

                )
                ? src.OwnerName ?? String.Empty
                : String.IsNullOrWhiteSpace(src.OwnerName) ? String.Empty : User.AnonymizedName;
    }
}
