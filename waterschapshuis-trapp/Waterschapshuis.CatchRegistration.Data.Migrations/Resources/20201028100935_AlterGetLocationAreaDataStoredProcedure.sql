...
AS
	...
	-- Catches in current week per catch type for a certain catch area.
	...
	AND C.RecordedOn between DATEADD(ww, DATEDIFF(ww,0,GETDATE()), 0) AND DATEADD(week, 1, DATEADD(ww, DATEDIFF(ww,0,GETDATE()), 0))
	GROUP BY CT.Name, CT.[Order]

	UNION ALL

	-- Catches in current week per catch type for a certain sub area.
	...
	-- By-catches in current week per catch type for a certain catch area.
	...
	-- By-catches in current week per catch type for a certain sub area.
	...
	-- Hours in current week for a certain catch area.
	...
	AND TR.Date between DATEADD(ww, DATEDIFF(ww,0,GETDATE()), 0) AND DATEADD(week, 1, DATEADD(ww, DATEDIFF(ww,0,GETDATE()), 0))

	UNION ALL

	-- Hours in current week for a certain sub area.
