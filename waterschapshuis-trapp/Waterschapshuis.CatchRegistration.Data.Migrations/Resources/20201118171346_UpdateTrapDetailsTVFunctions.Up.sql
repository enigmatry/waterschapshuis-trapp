...
(
	SELECT
		t.Id AS TrapId,
		SUM(
		CASE WHEN ct.IsByCatch = 0 AND ((@showPastYearCatchesOnly = 1 AND c.RecordedOn> DateAdd(dd, 1, DateAdd(yy, -1, GetDate())) AND c.RecordedOn < GETDATE() ) OR @showPastYearCatchesOnly = 0) THEN c.Number 
		ELSE 0
		END	) AS NumberOfCatches,
		SUM(CASE WHEN ct.IsByCatch = 1 AND ((@showPastYearCatchesOnly = 1 AND c.RecordedOn> DateAdd(dd, 1, DateAdd(yy, -1, GetDate())) AND c.RecordedOn < GETDATE() ) OR @showPastYearCatchesOnly = 0) THEN c.Number 
		ELSE 0
