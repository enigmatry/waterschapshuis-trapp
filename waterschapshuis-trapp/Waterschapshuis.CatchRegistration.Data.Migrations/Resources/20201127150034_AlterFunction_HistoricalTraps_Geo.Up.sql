...
(
	SELECT
		...
		OR (c.RecordedOn>= @catchStartDate AND  c.RecordedOn <=  @catchEndDate) )
		THEN c.Number 
		ELSE 0
		END	) AS NumberOfCatches,
		SUM(
		CASE WHEN ct.IsByCatch = 1 AND (c.RecordedOn>= @catchStartDate) AND (c.RecordedOn <= @catchEndDate) THEN c.Number 
		ELSE 0
	...
	AND (@trapEndDate is null OR @trapEndDate = 0 OR  t.CreatedOn <= @trapEndDate)
